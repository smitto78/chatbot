...
def render_rule_section():
    for key in ("rule_input", "qa_prompt_input", "qa_last_reply"):
        st.session_state.setdefault(key, "")

    if st.session_state["rule_input"]:
        if "qa_prompt_input" in st.session_state:
            st.session_state["qa_prompt_input"] = ""
        if "qa_last_reply" in st.session_state:
    ...
    if st.session_state.get("rule_lookup_result"):
